...
def sample_recon(nfnet, loader, orig_batch_siren):
    ...
    # new_outs = PGD(x=params, loss_fn=F.cross_entropy, y=orig_outs, model=nfnet, eps=0.01,
                            # steps=3, gamma=0.01, norm='linf', randinit=False, cuda=True, cnn=False, )
...
def compute_losses(x, x_recon, loss_type='l2'):
    if loss_type == 'l2':
        ...
        return torch.max(torch.abs(x_recon - x))
    else:
...
def PGD( \
    ...
    for t in range(steps):
        ...
        out_adv_tensor = tf.convert_to_tensor(out_adv_branch)
        if cnn:
            loss_adv = loss_fn(out_adv_tensor, y)
        else:
            loss_adv = loss_fn(out_adv_tensor[-1], y)
...
def train_and_eval(cfg):
    ...
    for step in outer_pbar:
        ...
        params = WeightSpaceFeatures(*wts_and_bs).to("cuda")
        wts_and_bs_adv = []
        for list in wts_and_bs:
            tensorList = []
            for tensor in list:
                x_adv = PGD(x=tensor, loss_fn=F.cross_entropy, y=img, model=nfnet, eps=0.01,
                            steps=3, gamma=0.01, norm='linf', randinit=False, cuda=True, cnn=False, )
                tensorList.append(x_adv)
            wts_and_bs_adv.append(tensorList)
        wts_and_bs = wts_and_bs_adv
        params = WeightSpaceFeatures(*wts_and_bs).to("cuda")
        ...
        with torch.amp.autocast("cuda", enabled=cfg.amp_enabled, dtype=amp_dtype):
            # pred_img = PGD(x=params, loss_fn=F.cross_entropy, y=img.cuda(), model=nfnet_fast, eps=0.01,
            #                 steps=3, gamma=0.01, norm='linf', randinit=False, cuda=True, cnn=False, )
